...
// Adds a word to the vocabulary
int ProvisionalAddWordToVocab(struct vocabulary *v, char *word) {
  //static long collide = 0;
  //static long nocollide = 0;
  int is_in = SearchVocab(v, word);
  if(is_in >= 0) {
    return is_in;
  }
  unsigned int hash, length = strlen(word) + 1;
  if (length > MAX_STRING) {
    printf("WARNING: truncating long word %s from %d to %d characters long",
	   word, length, MAX_STRING);
    length = MAX_STRING;
    word[length - 1] = '\0';
  }
  /* // Reallocate memory if needed */
  if ((v->vocab_size + 2) >= v->vocab_max_size) {
    v->vocab_max_size *= 2; /*+= 1000;*/
    v->vocab = (struct vocab_word *)realloc(v->vocab, v->vocab_max_size * sizeof(struct vocab_word));
  }
  hash = GetWordHash(v, word);
  // rehash until we find an empty spot
  while (v->vocab_hash[hash] != -1) {
    hash = (hash + 1) % vocab_hash_size;
  }
  v->vocab_hash[hash] = v->vocab_size;
  v->vocab[v->vocab_size].word = (char *)calloc(length, sizeof(char));
...
void SortAndReduceVocab(struct vocabulary *v, int min_count) {
  ...
  qsort(&(v->vocab[EOS_INDEX+1]), v->vocab_size - 1, sizeof(struct vocab_word), VocabCompare);
